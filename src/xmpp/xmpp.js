...
app.get("/client-names", (req, res) => {
  const clientNames = global.Clients.map((i) => i.displayName);
  console.log(`${clientNames}`);
  const response = `${clientNames}`;
  res.send(response);
});
  ...
  ws.on("message", async (message) => {
    ...
    switch (msg.root.name) {
      ...
      case "auth":
        ...
        if (!Utils.DecodeBase64(msg.root.content).includes("\u0000"))
          return Error(ws);
        let decodedBase64 = Utils.DecodeBase64(msg.root.content).split(
          "\u0000"
        );
        ...
        if (
          ...
          Authenticated = true;
          logger.xmpp(`Player with the name "${displayName}" has logged in.`);
          ws.send(
      ...
      case "presence":
        ...
        switch (msg.root.attributes.type) {
          case "unavailable":
            if (!msg.root.attributes.to) return;
            if (
              msg.root.attributes.to.endsWith(`@muc.${domain}`) ||
              msg.root.attributes.to.split("/")[0].endsWith(`@muc.${domain}`)
            ) {
        ...
        Utils.GetXMPPStatus(accountId, accountId, false);
        break;
    }
...
function RemoveClient(ws, joinedMUCs) {
  ...
  if (partyId && typeof partyId == "string") {
    global.Clients.forEach((ClientData) => {
      ...
      ClientData.client.send(
        XMLBuilder.create("message")
          .attribute("id", uuidv4().replace(/-/gi, "").toUpperCase())
  ...
  logger.xmpp(`Player with the name "${client.displayName}" has logged out.`);
}
...
function getMUCmember(roomName, displayName, accountId, resource) {
  return `${roomName}@muc.${domain}/${encodeURI(
    displayName
  )}:${accountId}:${resource}`;
}
